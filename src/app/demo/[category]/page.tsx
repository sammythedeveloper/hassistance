...
// }
"use client";
import { useState, useRef, useEffect, useContext, use } from "react"; // 1. Added 'use' here
...
const translations = {
  en: {
    initialPrompt: "Initializing Wellness Protocol.",
    placeholder: "Describe your concern...",
    assistantName: "Holistic AI Assistant",
    categories: { Physical: "Physical", Mental: "Mental", Emotional: "Emotional", Environmental: "Environmental" }
  },
  am: {
    initialPrompt: "የጤና ፕሮቶኮል በመጀመር ላይ።",
    placeholder: "ስጋትዎን ይግለጹ...",
    assistantName: "Holistic AI ረዳት",
    categories: { Physical: "አካላዊ", Mental: "አእምሮአዊ", Emotional: "ስሜታዊ", Environmental: "አካባቢያዊ" }
  }
};

// 2. Changed type to Promise<{ category: string }>
export default function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  // 3. 'use(params)' unwraps the promise to get your category
  ...
  const [loading, setLoading] = useState(false);
  const [chat, setChat] = useState<{ role: string; content: string }[]>([
    { role: "assistant", content: `Protocol **${category}** active. ${t.initialPrompt}` },
  ]);
  const scrollRef = useRef<HTMLDivElement>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ category, issue, userContext: config }),
      });
      ...
      await saveConversation("session", category, issue, fullResponse);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
        ...
        <div className="max-w-2xl mx-auto space-y-8">
          {chat.map((msg, i) => (
             <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
               <div className="max-w-[90%] p-6 rounded-2xl bg-zinc-900 border border-zinc-800">
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-zinc-800">
        <div className="max-w-2xl mx-auto relative">
          <Textarea 
            ...
            className="bg-zinc-900 border-zinc-800 resize-none pr-14"
          />
          <Button type="submit" disabled={loading} className="absolute right-2 bottom-2 rounded-xl">
